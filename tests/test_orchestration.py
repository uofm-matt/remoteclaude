...
class OrchestrationTest(unittest.TestCase):
    def setUp(self):
        ...
        rc_launcher.CLAUDE_PROJECTS = Path(
            self.tmp, "claude-projects"
    ...
    def _seed_desk_thread(self, proj: str) -> None:
        ...
        slug = rc_launcher.re.sub(
            r"[^A-Za-z0-9]", "-", os.path.join(rc_launcher.PARENT, proj)
        )
    ...
    def test_launch_cmd_resume_vs_fresh(self):
        ...
        self.assertEqual(
            rc_launcher.launch_cmd("proj")[0],
            [c, "--continue", "--fork-session", "--remote-control", "proj"],
        )
        rc_launcher.RESUME, rc_launcher.SPAWN = (
            "off",
            "same-dir",
    ...
    def test_has_desk_thread_discrimination_and_cap(self):
        ...
        self.assertFalse(
            rc_launcher.has_desk_thread("proj")
    ...
    def test_desk_stop_graceful_and_clears_cache(self):
        rc_launcher._desk_cache = (
            9e18,
            ["proj"],
        )  # a warm cache the stop must invalidate
        self.desk = {"111": desk(os.path.join(rc_launcher.PARENT, "proj"))}
    ...
    def test_desk_projects_finds_plain_claude_by_cwd(self):
        rc_launcher._desk_cache = (
            0.0,
            [],
        )  # reset the TTL cache; restored value irrelevant
        root = os.path.join(rc_launcher.PARENT, "proj")
        self.desk = {
