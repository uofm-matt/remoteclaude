...
class FunctionTest(unittest.TestCase):
    ...
    def test_projects_lists_valid_dirs_sorted(self):
        ...
        open(
            os.path.join(rc_launcher.PARENT, "afile"), "w"
        ).close()  # a file, not a dir
    ...
    def _git_repo(self, name: str, branch: str = "main") -> str:
        ...
        def g(*a):
            subprocess.run(
                [rc_launcher.GIT, "-C", path, *a], capture_output=True, check=True
            )
    ...
    def test_git_state_reports_exact_branch_and_dirty(self):
        ...
        self.assertTrue(
            rc_launcher._git_state("repo")["d"]
        )  # a tracked edit reads dirty
    ...
    def test_git_state_none_for_nonrepo(self):
        self._proj("plain")
        self.assertIsNone(
            rc_launcher._git_state("plain")
        )  # a bare dir isn't a work tree
    ...
    def test_session_states_ranks_and_expires(self):
        ...
        write(
            "s4", "working", "gamma", now - rc_state.STATE_TTL - 60
        )  # stale -> ignored
    ...
    def test_human_size_units(self):
        ...
        self.assertEqual(
            rc_launcher.human_size(1024**5), "1.0 PB"
    ...
    def test_crumb_html_encoded_rel_not_double_encoded(self):
        ...
        self.assertIn(
            '<a href="/files/my%20file">my file</a>', crumb
        )  # once-encoded, decoded label
        self.assertNotIn("%2520", crumb)
        page = rc_launcher.share_page(
    ...
    def test_rows_html_carries_sort_keys_dirs_first(self):
        ...
        self.assertLess(
            rows.index("sub/"), rows.index("a.txt")
    ...
    def test_page_placeholder_named_project_not_reinterpreted(self):
        ...
        rc_launcher.desk_projects = (
            lambda: []
        )  # else this unit test forks real pgrep/ps
    ...
    def test_session_states_skips_nonjson_bad_and_projectless(self):
        ...
        (rc_launcher.STATE_DIR / "bad.json").write_text(
            "{not json"
        )  # decode error -> skipped
        (rc_launcher.STATE_DIR / "np.json").write_text(
            '{"state":"working","ts":9e18}'
        )  # no project
