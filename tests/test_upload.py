...
class UploadServerTest(unittest.TestCase):
    ...
    def test_files_get_sets_auth_cookie(self):
        ...
        self.assertIn("SameSite=Strict", cookie)  # CSRF defense for the cookie-authed writes
    ...
    def test_cookie_authorizes_write_and_no_auth_is_403(self):
        ...
        status, _, _ = self.req("PUT", "/files/c.bin", body=b"hi",
                                headers={"X-Rc-Offset": "0", "X-Rc-Total": "2"})
        self.assertEqual(status, 200)
        status, _, _ = self.req("PUT", "/files/n.bin", body=b"x",
                                headers={"X-Rc-Offset": "0", "X-Rc-Total": "1"}, cookie=False)
        self.assertEqual(status, 403)

    def test_full_upload_finalizes_without_leftover(self):
        data = bytes(i % 256 for i in range(1000))
        status, _, body = self.req("PUT", "/files/f.bin", body=data,
    ...
    def test_resume_from_partial(self):
        data = bytes(i % 256 for i in range(1000))
        status, _, body = self.req("PUT", "/files/r.bin", body=data[:400],
                                   headers={"X-Rc-Offset": "0", "X-Rc-Total": "1000"})
        self.assertEqual(status, 200)
    ...
    def test_resume_identity_isolates_stale_partial(self):
        ...
        self.req("PUT", "/files/x.bin", body=b"A" * 100,
        ...
        full = b"B" * 500
        _, _, body = self.req("PUT", "/files/x.bin", body=full[have:],
    ...
    def test_gap_offset_returns_409(self):
        self.req("PUT", "/files/g.bin", body=b"x" * 100,
                 headers={"X-Rc-Offset": "0", "X-Rc-Total": "500"})
        status, _, body = self.req("PUT", "/files/g.bin", body=b"y" * 10,
    ...
    def test_malformed_headers_400_and_server_survives(self):
        for off in ("abc", "-5"):  # non-numeric used to crash the thread; negative was swallowed
            status, _, _ = self.req("PUT", "/files/b.bin", body=b"x",
    ...
    def test_delete_removes_file_and_refuses_root_and_dir(self):
        self.req("PUT", "/files/d.bin", body=b"x", headers={"X-Rc-Offset": "0", "X-Rc-Total": "1"})
        ...
        self.assertEqual(self.req("DELETE", "/files/..%2f..%2fx")[0], 403)  # a traversal escape
    ...
    def test_body_bearing_get_closes_connection(self):
        ...
        s.sendall(f"GET /files?token={TOKEN} HTTP/1.1\r\nHost: x\r\n"
                  f"Content-Length: 5\r\n\r\nHELLO".encode())
        resp = s.recv(65536).decode("latin1")
        s.close()
        self.assertIn(" 200 ", resp.splitlines()[0])      # the GET itself succeeded
        self.assertIn("connection: close", resp.lower())  # and the server closed the socket

    def test_rejected_request_is_logged(self):
        logged: list = []
        rc_launcher.log_event = lambda *a: logged.append(a)  # capture instead of silence
    ...
    def test_upload_write_error_is_logged_not_finalized(self):
        logged: list = []
        rc_launcher.log_event = lambda *a: logged.append(a)
        os.chmod(self.share, 0o500)  # read-only dir -> the .rcpart open() raises OSError
        try:
            status, _, body = self.req("PUT", "/files/e.bin", body=b"data",
    ...
    def test_put_unknown_path_404(self):
        status, _, _ = self.req("PUT", "/nope", body=b"x",
                                headers={"X-Rc-Offset": "0", "X-Rc-Total": "1"})
        self.assertEqual(status, 404)

    def test_upload_without_total_header_finalizes(self):
        status, _, body = self.req("PUT", "/files/nt.bin", body=b"12345",
                                   headers={"X-Rc-Offset": "0"})  # no X-Rc-Total -> defaults
        self.assertEqual(status, 200)
        self.assertTrue(json.loads(body)["done"])
    ...
    def test_upload_to_directory_is_bad_target(self):
        ...
        status, _, _ = self.req("PUT", "/files/adir", body=b"x",
                                headers={"X-Rc-Offset": "0", "X-Rc-Total": "1"})
        self.assertEqual(status, 403)

    def test_upload_to_missing_folder_404(self):
        status, _, _ = self.req("PUT", "/files/nodir/f.bin", body=b"x",
                                headers={"X-Rc-Offset": "0", "X-Rc-Total": "1"})
        self.assertEqual(status, 404)

    def test_files_traversal_get_404(self):
        status, _, _ = self.req("GET", f"/files/..%2f..%2fetc?token={TOKEN}", cookie=False)
        self.assertEqual(status, 404)
    ...
    def test_wrong_token_and_cookie_rejected(self):
        self.assertEqual(self.req("GET", "/files?token=wrongvalue", cookie=False)[0], 403)
        self.assertEqual(self.req("GET", "/files", cookie=False,
                                  headers={"Cookie": "rc_token=wrongvalue"})[0], 403)
    ...
    def test_no_auth_denied_on_every_verb(self):
        for verb in ("GET", "PUT", "DELETE", "HEAD"):
            ...
            self.assertEqual(self.req(verb, "/files/x", cookie=False, headers=hdr)[0], 403, verb)

    def test_fail_closed_when_no_token_configured(self):
        rc_launcher.TOKEN = ""  # misconfigured server: an empty token must deny, not match empty creds
        try:
            ...
            self.assertEqual(self.req("GET", "/files/x", cookie=False,
                                      headers={"Cookie": "rc_token="})[0], 403)
            # and a real-looking token is still rejected when the server holds none
            self.assertEqual(self.req("GET", f"/files/x?token={TOKEN}", cookie=False)[0], 403)
        finally:
            rc_launcher.TOKEN = TOKEN
    ...
    def test_dropped_connection_keeps_partial(self):
        ...
        pattern = bytes(range(256)) * 300  # 76800 distinctive bytes: catches stale-length/corruption
        s = socket.create_connection(("127.0.0.1", self.port), timeout=5)
        s.sendall(f"PUT /files/drop.bin HTTP/1.1\r\nHost: x\r\nCookie: rc_token={TOKEN}\r\n"
        ...
        self.assertGreaterEqual(have, 65536)  # partial survived the drop (ConnectionError branch)
        kept = Path(self.share, "drop.bin.rcpart").read_bytes()
        self.assertEqual(kept, pattern[:have])  # exact prefix of what was sent, not garbage/stale
...
class RowsHtmlTest(unittest.TestCase):
    ...
    def test_hides_rcpart_and_escaping_symlink(self):
        ...
        os.symlink(tempfile.mkdtemp(), os.path.join(self.share, "escape"))  # -> outside SHARE
    ...
    def test_rows_html_dirs_first_empty_and_unreadable(self):
        ...
        rows = rc_launcher.rows_html(self.share, "")
        self.assertLess(rows.index("adir"), rows.index("afile.txt"))  # dirs before files
        self.assertIn("empty", rc_launcher.rows_html(os.path.join(self.share, "adir"), "/adir"))
        self.assertIn("empty", rc_launcher.rows_html(os.path.join(self.share, "no"), "/no"))  # OSError
...
class RouteTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        rc_launcher.RESUME = "off"  # fresh launches (no takeover) keep route tests simple
